var App = function () {
    ...
    function show_news(data) {
        var news = data['oprecord'];
        $("#total_news_num").text(news.length)
        $("#total_news_info").text(news.length + "个任务需要操作")
        $("#task_bar").text(news.length);
        $("#task-news").empty()
        $("#show_task_bar_conut").html(news.length + "个当前任务");
        $("#progress_bar").empty()
        var show_content = '<li class="dropdown-title"> ' +
								'<span><i class="fa fa-bell" id="total_news_info"></i>' + news.length + '个任务需要操作' +'</span>' +
							'</li>'
        $("#task-news").append(show_content)
        for(var i=0;i<news.length;i++){
            var operate = ''
            var progress_bar = ''
            if(news[i][3] == 1){
                if(news[i][2]){
                    operate = '<span class="label label-success" style="padding-top: 13px">审核</span>'
                    counts =20
                    status = '等待审核！任务结束还需四步'
                    task_name = news[i][1]

                }else {
                    operate = '<span class="label label-success" style="padding-top: 13px">发布</span>'
                    counts =40
                    status = '等待发布！任务结束还需三步'
                    task_name = news[i][1]
                }

            }else if(news[i][3] == 2){
                operate = '<span class="label label-success" style="padding-top: 13px">确认</span>'
                counts =80
                status = '等待确认！任务结束还需一步'
                task_name = news[i][1]
            }else if(news[i][3] == 5){
                operate = '<span class="label label-success" style="padding-top: 13px">完成</span>'
                counts =60
                status = '等待发布完成！任务结束还需两部步'
                task_name = news[i][1]
            }else if(news[i][3] == 6){
                operate = '<span class="label label-success" style="padding-top: 13px">发布</span>'
                counts =40
            ...
            show_content = '<li> <a href="/upload/detailpage/?versionid=' +
                news[i][0] +
                '&page=">' +
                 operate +
                '<span class="body">' +
                ...
                '</span></span></a></li>'
            $("#task-news").append(show_content)
            progress_bar = '<li><span class="header clearfix"><span class="pull-left">'+
                task_name+
                '</span><span class="pull-right">'+
                counts+
                '%</span></span>'+
               '<i style="font-size: 6px">当前流程：'+
                status+'</i>'+
                '<div class="progress progress-striped active"><div class="progress-bar progress-bar-success" style="width:'+
                counts+
                '%"><span class="sr-only">'+

                '</span></div>'+
               '</div></li>'
            $("#progress_bar").append(progress_bar)

        };
    };

    function get_news_result() {
        var data ={};
        $.ajax({
            type: "GET",
            url: "/adminset/news",
            ...
            success: function(data) {
                show_news(data)
                setTimeout(get_news_result, 2000);
            },
            dataType: 'json'
        });
    };
    ...
    function sidebar_open (obj)  {
            //alert(obj)
            var last = jQuery('.has-sub.open', $('.sidebar-menu'));
            last.removeClass("open");
            jQuery('.arrow', last).removeClass("open");
            jQuery('.sub', last).slideUp();

            var sub = jQuery(obj).next();
            if (sub.is(":visible")) {
                jQuery('.arrow', jQuery(ojb)).removeClass("open");
                jQuery(obj).parent().removeClass("open");
				sub.slideUp(-1);

            } else {
                jQuery('.arrow', jQuery(obj)).addClass("open");
                jQuery(obj).parent().addClass("open");
                sub.slideDown(-1);
            }
         };

        //alert($.cookie('sidebar_open'))
        //if ($.cookie('sidebar') === "task" || $.cookie('sidebar') === "project" || $.cookie('sidebar') === "access"){
        if ($.cookie('sidebar_open') != null){
            //alert($.cookie('sidebar_open'))
           var id = $.cookie('sidebar_open');
            //alert("#"+id)
            sidebar_open("#"+id);
        }
	...
	jQuery('.sidebar-menu .has-sub > a').click(function () {
            ...
            //alert(id_value)
            $.cookie('sidebar_open', id_value, { path: '/', expires: 10 });
            //alert($.cookie('sidebar_open'))
    ...
    jQuery('.sidebar-menu .no-sub > a').click(function () {
            $.cookie('sidebar_open', '', { path: '/', expires: 10 });
        });
	...
	jQuery('.sidebar-menu .has-sub .sub .has-sub-sub > a').click(function () {
            ...
            jQuery('.sub', last).slideUp(200);
            var id_value = jQuery(this).attr('id')
            $.cookie('sidebar', id_value, { path: '/', expires: 10 });
            var sub = jQuery(this).next();
            if (sub.is(":visible")) {
                jQuery('.arrow', jQuery(this)).removeClass("open");
                jQuery(this).parent().removeClass("open");
	...
	var handleTeamView = function () {
		...
		$(".team-status-toggle").click(function (y) {
            //console.log('触发team-status-toggle')
            y.preventDefault();
            w(this);
